<template>
    <div>
        <Head
            :title="form.id ? $t('purchases.edit') : $t('purchases.create')"
        />
        <div class="card">
            <div class="card-header bg-white d-flex">
                <h5 class="mb-0">
                    {{
                        form.id ? $t("purchases.edit") : $t("purchases.create")
                    }}
                </h5>
                <Link
...
const supplierOpts = computed(() =>
    props.options.suppliers.map((s) => ({ value: s.id, label: s.name })),
);
...
const form = useForm({
    ...
    purchase_date:
        props.purchase?.purchase_date ||
        new Date().toISOString().substring(0, 10),
    status: props.purchase?.status || "received",
...
const totals = computed(() => {
    ...
    for (const it of form.items) {
        const sub =
            Number(it.quantity || 0) * Number(it.cost_price || 0) -
