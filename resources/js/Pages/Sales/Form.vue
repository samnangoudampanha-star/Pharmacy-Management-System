    ...
    <div>
        ...
        <div class="card">
            <div class="card-header bg-white d-flex">
                <h5 class="mb-0">{{ form.id ? $t('sales.edit') : $t('sales.create') }}</h5>
                <Link :href="route('admin.sales.index')" class="btn btn-outline-secondary btn-sm ms-auto">
            ...
            <form class="card-body" @submit.prevent="submit">
                <div class="row g-3 mb-3">
                    <div class="col-md-3">
                        <label class="form-label">{{ $t('fields.invoice_number') }}</label>
                    ...
                    <div class="col-md-3">
                        ...
                        <TomSelectInput v-model="form.customer_id" :options="customerOpts" :placeholder="$t('sales.walkin')" />
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Date <span class="text-danger">*</span></label>
                        <FlatpickrInput v-model="form.sale_date" date-format="Y-m-d" />
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ $t('fields.payment_method') }}</label>
                        <TomSelectInput v-model="form.payment_method" :options="paymentMethodOpts" />
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ $t('fields.paid') }}</label>
                        <input v-model.number="form.paid" type="number" step="0.01" class="form-control" />
                    </div>
                </div>
                    ...
                    <table class="table table-bordered align-middle">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th class="text-end" style="width: 130px;">{{ $t('fields.sale_price') }}</th>
                        ...
                        <tbody>
                            <tr v-for="(item, idx) in form.items" :key="idx">
                                ...
                                <td><input v-model.number="item.sale_price" type="number" step="0.0001" class="form-control text-end" /></td>
                        ...
                        <tfoot>
                            ...
                            <tr><td colspan="5" class="text-end fw-semibold">{{ $t('fields.subtotal') }}</td><td class="text-end">{{ format(totals.subtotal) }}</td><td></td></tr>
                            <tr><td colspan="5" class="text-end fw-semibold">{{ $t('fields.tax') }}</td><td class="text-end">{{ format(totals.tax) }}</td><td></td></tr>
                            <tr><td colspan="5" class="text-end fw-bold">{{ $t('fields.total') }}</td><td class="text-end fw-bold">{{ format(totals.total) }}</td><td></td></tr>
                            <tr><td colspan="5" class="text-end fw-semibold">{{ $t('fields.due') }}</td><td class="text-end fw-bold text-danger">{{ format(Math.max(0, totals.total - Number(form.paid || 0))) }}</td><td></td></tr>
                        </tfoot>
                    </table>
                </div>
                ...
                <div class="mt-4 d-flex gap-2">
                    ...
                    <Link :href="route('admin.sales.index')" class="btn btn-outline-secondary">{{ $t('common.cancel') }}</Link>
                </div>
            </form>
        </div>
    </div>
...
const paymentMethodOpts = (props.options.payment_methods || []).map((m) => ({ value: m, label: m }));
...
const form = useForm({
    ...
    sale_date: props.sale?.sale_date || new Date().toISOString().substring(0, 10),
...
function emptyLine() {
    return { product_id: null, quantity: 1, sale_price: 0, tax_rate: 0, discount: 0 };
}
...
function lineTotal(item) {
    const sub = (Number(item.quantity || 0) * Number(item.sale_price || 0)) - Number(item.discount || 0);
...
const totals = computed(() => {
    ...
    for (const it of form.items) {
        const sub = (Number(it.quantity || 0) * Number(it.sale_price || 0)) - Number(it.discount || 0);
        subtotal += sub;
