    ...
    <div>
        ...
        <div class="card">
            <div class="card-header bg-white d-flex">
                <h5 class="mb-0">{{ $t('menu.manufacturers') }}</h5>
                <Link :href="route('admin.manufacturers.index')" class="btn btn-outline-secondary btn-sm ms-auto">
            ...
            <form class="card-body" @submit.prevent="submit">
                ...
                <div class="mt-4 d-flex gap-2">
                    ...
                    <Link :href="route('admin.manufacturers.index')" class="btn btn-outline-secondary">{{ $t('common.cancel') }}</Link>
                </div>
            </form>
        </div>
    </div>
...
const form = useForm({
    id: props.manufacturer?.id || null,
    name: props.manufacturer?.name || '',
    country: props.manufacturer?.country || '',
    phone: props.manufacturer?.phone || '',
    email: props.manufacturer?.email || '',
    address: props.manufacturer?.address || '',
    is_active: props.manufacturer?.is_active ?? true,
});

function submit() {
    if (form.id) form.put(route('admin.manufacturers.update', form.id));
    else form.post(route('admin.manufacturers.store'));
}
</script>
